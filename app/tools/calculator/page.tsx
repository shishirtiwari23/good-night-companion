...
export default function CalculatorPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4 md:p-8 relative bg-cream text-ink">
      <div className="card-container w-full max-w-2xl relative p-6 md:p-12">
        ...
        <div className="relative mb-8">
          <div className="absolute -top-2 right-0 z-50">
            <Link
              ...
              className="text-ink-2 hover:text-[var(--primary)] transition-colors p-2 block"
            >
              <svg
        ...
        {/* --- INFO BOX --- */}
        <div className="bg-cream-2 rounded-xl p-6 mb-8 text-sm text-ink-2 space-y-2">
          <p>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-12 gap-4 text-[10px] font-bold tracking-[0.16em] uppercase text-ink-2 opacity-60 mb-2 px-1">
          ...
          {days.map((day, index) => {
            ...
            return (
              <div key={index} className="grid grid-cols-12 gap-4 items-center">
                <div className="col-span-2 text-sm md:text-base font-medium text-ink-2">
                  Day {index + 1}
                </div>
                ...
                <div className="col-span-5">
                  <input
                    ...
                    step="0.25"
                    placeholder="8.0"
                    className="w-full text-center p-3 bg-cream border border-rule rounded-lg focus:outline-none focus:border-[var(--primary)] text-ink font-serif font-medium text-lg shadow-sm placeholder:text-gray-300"
                    value={day.tib}
                    onChange={(e) =>
                ...
                <div className="col-span-5 relative">
                  <input
                    ...
                    step="0.25"
                    placeholder="7.5"
                    className={`w-full text-center p-3 bg-cream border rounded-lg focus:outline-none focus:border-[var(--primary)] text-ink font-serif font-medium text-lg shadow-sm placeholder:text-gray-300 ${
                      isInvalid
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-10 mb-4">
          <button
            ...
            className="w-full sm:w-1/2 py-4 bg-cream-2 hover:bg-[#dfddcf] text-ink rounded-full font-bold uppercase tracking-[0.16em] text-xs shadow-sm transition-all active:scale-99 cursor-pointer"
          >
            See Averages Only
          </button>
          ...
          <button
            ...
            className="w-full sm:w-1/2 py-4 bg-[#94A396] hover:bg-[#829285] text-white rounded-full font-bold uppercase tracking-[0.16em] text-xs shadow-md transition-all active:scale-99 cursor-pointer"
          >
            Calculate Score
          </button>
        </div>
          ...
          >
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-[#F0EEE6] rounded-xl p-4 text-center flex flex-col justify-center min-h-[90px]">
                <span className="font-serif font-medium text-2xl text-ink">
                  {result.daysLogged}
                </span>
                <span className="text-[10px] font-bold uppercase tracking-wider text-ink-2 opacity-70 leading-tight mt-1">
                  Days
                  <br />
                  Logged
                </span>
            ...
            {result.score !== null && !result.isError && (
              <div className="text-center py-4">
                <p className="text-xs font-bold uppercase tracking-wider text-ink-2 mb-2 opacity-60">
                  Sleep Efficiency (SE)
                </p>
                <div className="text-7xl font-serif font-medium text-[#9CA39B] tracking-tight">
                  {result.score}
            ...
            {(result.title || result.feedback) && (
              <div className="mt-8">
                <p className="text-lg text-ink-2 mb-4 font-sans">
                  Insights
                </p>
                <div
                  className={`bg-cream rounded-2xl p-8 border-l-4 shadow-sm ${result.isError ? "border-l-[#D4B483]" : "border-l-[#8da399]"}`}
                >
                  <h3 className="font-serif font-medium text-2xl text-ink mb-3">
                    {result.title}
                  </h3>
                  <p className="text-sm md:text-base text-ink-2 leading-relaxed font-semibold">
                    {result.feedback}
                  </p>

                  {result.hasSafetyWarning && (
                    <div className="mt-6 pt-4 border-t border-red-100">
                      <p className="text-xs font-bold uppercase tracking-[0.16em] text-red-500 mb-1">
                        Safety Warning
                      </p>
                      <p className="text-sm text-ink-2">
        ...
        <div className="mt-16 pt-8 border-t border-rule">
          <div className="bg-cream-2 rounded-xl p-6 border border-rule text-xs text-ink-2 leading-relaxed">
            <p className="font-bold text-ink uppercase tracking-wider mb-2">
              Medical Disclaimer & Safety Warning
            </p>
            <p className="mb-2">
              This tool utilizes Sleep Restriction Therapy (SRT) concepts. This technique is <strong className="text-ink">NOT</strong> recommended for individuals with Bipolar Disorder, Seizure Disorders, untreated Sleep Apnea, or those in safety-sensitive occupations (e.g., transportation).
            </p>
            <p>
